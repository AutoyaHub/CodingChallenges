...
if __name__ == "__main__":
    ...
    k = 3
    logging.info(
        f"Probability that x is divisible by {k} for s={s}: {probability_divisible_by_k(k, s)}"
    )
    logging.info(
        f"Probability that x is divisible by both 2 and 3 for s={s}: {probability_intersection_e2_e3(s)}"
    )
    logging.info(
        f"Probability of intersection over first 100 primes for s={s}: {probability_intersection_all_primes(s)}"
    )
